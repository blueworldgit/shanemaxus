...
<?php if (is_singular('post') || is_singular('product')): ?>
    <div class="title-section">
        <div class="container">
            ...
            <?php endif ?>
            <?php
            if ( ! empty( $_COOKIE['mvp_vehicle_slug'] ) && ! empty( $_COOKIE['mvp_vehicle_model'] ) ) :
                $mvp_model    = sanitize_text_field( wp_unslash( $_COOKIE['mvp_vehicle_model'] ) );
                $mvp_year     = ! empty( $_COOKIE['mvp_vehicle_year'] ) ? sanitize_text_field( wp_unslash( $_COOKIE['mvp_vehicle_year'] ) ) : '';
                $mvp_home_url = esc_js( home_url( '/#mvp-vehicles' ) );
            ?>
            <div class="mvp-vehicle-notice" style="background:#0a1628;color:#fff;padding:8px 16px;font-size:13px;display:flex;align-items:center;gap:12px;">
                <span>Viewing parts for: <strong><?php echo esc_html( $mvp_model ); ?><?php if ( $mvp_year ) echo ' (' . esc_html( $mvp_year ) . ')'; ?></strong></span>
                <a href="<?php echo $mvp_home_url; ?>" onclick="mvpClearVehicleCookies();return false;" style="color:#e8b84b;text-decoration:none;">&#8635; Change vehicle</a>
            </div>
            <script>
            if (typeof window.mvpClearVehicleCookies === 'undefined') {
                window.mvpClearVehicleCookies = function() {
                    var past = 'expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/';
                    document.cookie = 'mvp_vehicle_slug=; ' + past;
                    document.cookie = 'mvp_vehicle_serial=; ' + past;
                    document.cookie = 'mvp_vehicle_model=; ' + past;
                    document.cookie = 'mvp_vehicle_year=; ' + past;
                    window.location.href = '<?php echo $mvp_home_url; ?>';
                };
            }
            </script>
            <?php endif; ?>
